...
import {
  ...
  Speciality,
} from '../../utils/SemesterSection.types';
...
const SemesterSection = ({ year }: SemesterSectionProps) => {
  ...
  const semesters =
    selectedSpecialityIndex >= 0
      ? selectedYear.specialities[selectedSpecialityIndex].semesters
      : [];
  const semesterInstances = semesters.map((semesterData) => {
    const subjects = semesterData.subjects.map(
      (subjectData) => new Subject(subjectData.name, subjectData.link)
    );
  ...
  return (
    <div
      ...
      id={selectedYear.year.split(' ').join('-')}
    >
      <h1 className="year-title">{selectedYear.year}</h1>

      <div className="speciality-selection">
        {selectedYear.specialities.map(
          (speciality: Speciality, specialityIndex: number) => (
            <button
              key={speciality.speciality}
              className={`speciality-button ${
                selectedSpecialityIndex === specialityIndex ? 'selected' : ''
              }`}
      ...
      <div className="slider">
        ...
        <img
          className={`iconimg ${
            index === semesterInstances.length - 1 ? 'inactive' : ''
          }`}
